...
room ids, defined in the "room-map" text field of this plugin.  See comments for that field below
for details.
...
wrap.splice(BipsiPlayback.prototype, 'move', (original, dx, dy) => {
	if (!PLAYBACK.canMove || PLAYBACK.busy) return; // Extra "busy" flag check for compatibility with "smooth-move" plugin.
	const avatar = getEventById(PLAYBACK.data, PLAYBACK.avatarId);
	const [px, py] = avatar.position;
	const [tx, ty] = [px+dx, py+dy];
	const currentRoomAdjacencies = roomAdjacencies[roomFromEvent(PLAYBACK.data, avatar).id];
	...
	if (adjacencyDestination) {
		const room = getRoomById(PLAYBACK.data, adjacencyDestination.room);
		if (cellIsSolid(room, adjacencyDestination.target[0], adjacencyDestination.target[1])) return;
		moveEvent(PLAYBACK.data, avatar, adjacencyDestination);
	}
	// Finish with normal movement logic
	original.bind(PLAYBACK)(dx, dy);
});
